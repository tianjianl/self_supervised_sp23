...
class OPTModelForSequenceClassification(nn.Module):
    ...
    def __init__(self, model_name, num_labels, student_layer):
        ...
        hidden_size = 2048 # facebook/opt-1.3b - hidden size: 2048
    ...
    def forward(self, input_ids, attention_mask):
        ...
        #last hidden state shape: batch size, length, hidden size
...
class CustomClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        src = ' '.join(src.split())
        source = self.tokenizer.batch_encode_plus([src], 
...
def train(epoch, tokenizer, model, device, loader, optimizer, args, scheduler=None, regularizer=None):
    ...
    for iteration, data in enumerate(loader, 0):
        ...
        #print(f'forward used {forward_time - before_forward} seconds')

        loss.backward() # backprop gradient
        #backward_time = time.time()
        #print(f'backward used {backward_time - forward_time} seconds') 
...
def main(args):
    ...
    wandb.init(project=f"opt-1b-glue-distillation", 
            entity="dogtooooth", name=wandb_name)

    for k, v in vars(args).items():
        print(f"{k}: {v}")
    
    num_labels = label_dict[args.task]
    model = OPTModelForSequenceClassification(args.model_name, num_labels, args.student_layer)
    model.to(device)
...
if __name__ == "__main__":
    ...
    os.environ['TRANSFORMERS_CACHE'] = '/scratch/tli104/transformers_cache'
    ...
    parser.add_argument("--model_name", default='facebook/opt-1.3b')
